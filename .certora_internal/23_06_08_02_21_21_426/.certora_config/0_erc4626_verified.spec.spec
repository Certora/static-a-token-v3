...
rule checkMaxSharesRedeem(
    ...
    require _SymbolicLendingPool.reserveIsActive() == false;
...
rule checkWithdrawalCorrectnessWithDeposit(
    ...
    require _SymbolicLendingPool.getReserveNormalizedIncome(e,getStaticATokenUnderlying()) == 1 ||
            _SymbolicLendingPool.getReserveNormalizedIncome(e,getStaticATokenUnderlying()) == 2;
...
rule checkWithdrawCorrectnessWhenDeposit(
    ...
    assert totalAssetsBefore < totalAssetsAfter, "assets were lost";
}
...
rule ValidAmountShouldBeDepositedOnMint(
    ...
    uint256 aTokenBalanceBefore = _AToken.balanceOf(e, currentContract);
    uint256 aTokenBalanceOfMinterBefore = _AToken.balanceOf(e, e.msg.sender);
    uint256 totalAssetsBefore = totalAssets(e);
    ...
    uint256 aTokenBalanceAfter = _AToken.balanceOf(e, currentContract);
    uint256 aTokenBalanceOfMinterAfter = _AToken.balanceOf(e, e.msg.sender);
    uint256 totalAssetsAfter = totalAssets(e);
    ...
    assert ( aTokenBalanceBefore + assetsAmount == aTokenBalanceAfter &&
...
rule correctAmountOnPreviewMint(
    ...
    uint256 actualMintAmount = mint(e, sharesAmount, receiver) at init_state;
