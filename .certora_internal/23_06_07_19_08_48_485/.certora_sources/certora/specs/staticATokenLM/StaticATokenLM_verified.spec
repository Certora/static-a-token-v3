...
rule checkCorrectnessOfMaxRedeem(
    ...
    uint256 maxSharesAmountBefore = maxRedeemUnderlying(e, owner) at init_state;
    uint256 maxSharesAmountAfter = maxRedeemUnderlying(e, owner) at init_state;
...
rule CannotRedeemMoreUnderlyingThanMaxRedeemUnderlying(
    ...
    uint256 senderSharesBasedOnUnderlyingBalance = convertToAssets(e, shares);
    ...
    if( senderUnderlyingBalance <= aTokenContractUnderlyingBalance ||
        ...
        sharesAmount, assetsAmount = redeem(e, sharesAmount, receiver, addr, toUnderlying) at init_state;
        ...
        sharesAmount, assetsAmount = redeem(e, shares, recipient, owner, toUnderlying) at init_state;

        assert (sharesAmount <= maximumSharesAmount) || (assetsAmount <= maximumAssetsAmount) || (sharesAmount <= 1 && assetsAmount <=1);
    }
}
